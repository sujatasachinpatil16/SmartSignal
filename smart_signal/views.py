...
from .models import Client,signal_details,Signals
...
from .forms import AddRoadCameraForm
...
def add_road_camera(request):
    context = {}
    form = AddRoadCameraForm(request.POST or None)
   # context['form'] = form
    if request.method == "POST":
       # form = AddRoadCameraForm(request.POST)
        if form.is_valid():
                Camera = form.save(commit=False)
                Camera.road_number = request.user
                Camera.base_time = request.user
                Camera.camera_ip = request.user
                Camera.camera_url = request.user
                Camera.priority = request.user
                Camera.on_time = request.user
                Camera.off_time = request.user
                Camera = Camera.save()
                ...
                print("in else part")
                form = AddRoadCameraForm()
    return render(request,'add_road_camera.html',context)

def showroadcamera(request):
    obj = Signals.objects.all()
    return render(request,'show_road_camera.html',{'data':obj})
...
#   print(cmd1)
    ...
    try:
       ...
       logger.exception("exception for command to get city and location for current user:")
    ...
    reader = list(reader1)
   # print("reader: ", reader)
    logger.exception("city1.csv reader after converting to list:")
    ...
    #print('value',values)
    logger.exception("Final values of city_names:")
...
def get_all_clients_and_signals():
    ...
    for client in clidetails:
        ...
        cmd1 = cmd + ' > city.txt'
     #   print(cmd1)
        logger.exception("command for getting all clients and signal:")
        res = os.popen(cmd1)
        res1 = res.read()
     #   print('res1 :', res1)
        df = pd.read_csv("city.txt", delimiter=',')
...
def update_csv(city,video):
    ...
    upload_csv_to_s3 = "aws s3 cp " + filename_processed_str + " " + "s3://all-signal-csv-files"
...
def get_date_of_upload(cnm,location,pri_no_pri,timestamp,video_length,res1,video_title):
    print('res1 in get_videos:', res1)
    print('cnm ,location,pri_no_pri in get_videos: ',cnm,location,pri_no_pri,video_title)
    print("timestamp and video_len:",timestamp,video_length)
    dt_and_size = []
    num_rows = len(res1) - 1
    ...
    for j in range(num_rows):
        ...
        temp = a[j][4]
        t1 = cnm + '/' + location + '/' + pri_no_pri+'/'+timestamp + "/" + video_title
        val = temp in t1
        print("temp t1 val:",temp,t1,val)
        t = ''
        tt, dt = time_conversion(a[j][1], a[j][2])
        print("Time after conversion: ",tt)
        a[j][1] = dt
        a[j][2] = tt
        a[j][3] = round(mb)
        if val:
            t = temp.replace(t1, ' ')
        a[j][4] = video_title
        s1,s2 = video_title.split("/")
        print("t and video_title s2: ",t,video_title,s2)
        t = t.strip()
        if t == s2:
    #    print("both are same")
    ...
    print("dt_and_size: ",dt_and_size)
    return dt_and_size

def remove_duplicated_records(model, fields):
    """ Removes records from `model` duplicated on `fields`  while leaving the most recent one (biggest `id`)."""
...
#    print("duplicates after filtering: ", duplicates)
    for duplicate in duplicates:
        ...
        # leave out the latest duplicated record, you can use `Min` if you wish to leave out the first record
...
def write_record_to_csv(location,information):
    csv_columns = ['Cnm', 'Location', 'Created_on','Created_at_time','Size_of_video','Video_title','Timeslot','Subscription_time']
#    print("location in write_record_to_csv:",location)
...
@csrf_exempt
def uploadvideo_at_all_sites_old(request):
    str1 = "aws s3 cp "
    city_names = []
    values = get_city_and_location(request.user.username)
    cnm = values['company_name']
